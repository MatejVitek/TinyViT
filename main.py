...
def parse_option():
	parser = argparse.ArgumentParser(
		'ScleraViT training and evaluation script', add_help=False)
	add_common_args(parser)
	args = parser.parse_args()

	if isinstance(args.cfg, str):
		# Automatically set paths to config and pretrained weights if shorthand 5m/11m/21m or shorthand XXX is passed to --cfg
		match = re.fullmatch(r'([sStT]?)(\d+[mM]?)', args.cfg)
		if match:
			if args.pretrained is None:
				args.pretrained = EYEZ/f'Recognition/Models/TinyViT/TinyViT-{match.group(2).upper()}/default/ckpt_best.pth'
				if not args.pretrained.is_file():
					args.pretrained = args.pretrained.parent.parent/'original.pth'
			model_dir = 'sclera' if match.group(1) == 's' else 'tiny'
			args.cfg = Path(f'configs/matej/{model_dir}vit/{match.group(2).lower()}.yaml')
		else:
			# Automatically set paths to config and pretrained weights if shorthand XXX-YYY is passed to --cfg
			match = re.fullmatch(r'([sStT]?)(\d+)(-\d+)', args.cfg)
			if match:
				model_cls = 'Sclera' if match.group(1) == 's' else 'Tiny'
				if args.pretrained is None:
					args.pretrained = EYEZ/f'Recognition/Models/TinyViT/{model_cls}ViT-{match.group(2)}/default/ckpt_best.pth'
					if not args.pretrained.is_file():
						args.pretrained = EYEZ/f'Recognition/Models/TinyViT/TinyViT-{match.group(2)}/original.pth'
				args.cfg = Path(f'configs/matej/{model_cls.lower()}vit/{match.group(2)}{match.group(3)}.yaml')
...
def main(args, config):
	...
	if not args.only_cpu:
		try:
			...
			logger.info("Not using distributed training, so DistributedDataParallel will not be used")
	...
	if 'sclera' in config.MODEL.TYPE.lower():
	...
	if config.TRAIN.AUTO_RESUME:
		if not config.SAVE_FREQ:
			print("Attempting to resume from last checkpoint ...")
			resume_file = Path(config.OUTPUT)/'ckpt_final.pth'
			if not resume_file.is_file():
				print("Failed, attempting to resume from best checkpoint ...")
				resume_file = Path(config.OUTPUT)/'ckpt_best_loss.pth'
				if not resume_file.is_file():
					resume_file = Path(config.OUTPUT)/'ckpt_best.pth'
		if config.SAVE_FREQ or not resume_file.is_file():
	...
	if config.MODEL.RESUME:
		max_accuracy, min_loss = load_checkpoint(
		...
		if dist.get_rank() == 0:
			if config.SAVE_FREQ:
				if epoch % config.SAVE_FREQ == 0 or epoch == (config.TRAIN.EPOCHS - 1):
					save_checkpoint(config, epoch, model_without_ddp, max_accuracy, min_loss,
					                optimizer, lr_scheduler, loss_scaler, logger)
			else:
				if acc1 >= max_accuracy:
					save_checkpoint(config, epoch, model_without_ddp, max_accuracy, min_loss,
					                optimizer, lr_scheduler, loss_scaler, logger, 'best')
				if loss <= min_loss:
					save_checkpoint(config, epoch, model_without_ddp, max_accuracy, min_loss,
					                optimizer, lr_scheduler, loss_scaler, logger, 'best_loss')
				save_checkpoint(config, epoch, model_without_ddp, max_accuracy, min_loss,
				                optimizer, lr_scheduler, loss_scaler, logger, 'final')
...
def train_one_epoch(args, config, model, criterion, data_loader, optimizer, epoch, mixup_fn, lr_scheduler, loss_scaler):
	...
	if 'sclera' in config.MODEL.TYPE.lower():
		criterion, gaze_criterion = criterion
	...
	for idx, (samples, targets) in enumerate(data_loader):
		...
		if 'sclera' in config.MODEL.TYPE.lower():
			targets, gaze_targets = targets

		if not args.only_cpu:
			samples = samples.cuda(non_blocking=True)
			targets = targets.cuda(non_blocking=True)
			if 'sclera' in config.MODEL.TYPE.lower():
				gaze_targets = gaze_targets.cuda(non_blocking=True)
		...
		with torch.cuda.amp.autocast(enabled=config.AMP_ENABLE):
			...
			if 'sclera' in config.MODEL.TYPE.lower():
				outputs, gaze_outputs = outputs
				gaze_loss = gaze_criterion(gaze_outputs, gaze_targets)
			else:
				gaze_loss = 0
			loss = args.alpha * criterion(outputs, targets) + (1 - args.alpha) * gaze_loss
...
def validate(args, config, data_loader, model, num_classes=1000):
	...
	for idx, (images, target) in enumerate(data_loader):
		if 'sclera' in config.MODEL.TYPE.lower():
			target = target[0]  # In validation we don't care about gaze prediction accuracy
		if not args.only_cpu:
		...
		with torch.cuda.amp.autocast(enabled=config.AMP_ENABLE):
			output = model(images)
			if 'sclera' in config.MODEL.TYPE.lower():
				output = output[0]  # In validation we don't care about gaze prediction accuracy
		if num_classes == 1000:
...
if __name__ == '__main__':
	...
	if config.DISTILL.TEACHER_LOGITS_PATH:
		if config.DISTILL.TEACHER_LOGITS_PATH.lower() == 'default':
			config.DISTILL.TEACHER_LOGITS_PATH = EYEZ/'Recognition/Models/ScleraViT/teacher'
	...
	if args.only_cpu or platform.system().lower() == 'windows':  # No NCCL on Windows :(
		ddp_backend = 'gloo'
	else:
	...
	if is_main_process():
		...
		if args.use_wandb:
			...
			wandb.init(project="ScleraViT", config=config_dict,
